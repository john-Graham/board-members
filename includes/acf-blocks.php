...
add_action( 'init', 'coe_boards_blocks_register', 5 );
add_filter( 'block_categories_all', 'coe_boards_block_category' );
add_filter( 'acf/blocks/no_fields_assigned_message', 'coe_boards_block_no_fields_msg', 10, 2 );
 ...
 * @since 0.1.1
 */
function coe_boards_blocks_register() {
	$blocks = coe_boards_get_blocks();
	...
	foreach ( $blocks as $block ) {
		if ( file_exists( COE_BOARDS_PLUGIN_BLOCKS . $block . '/block.json' ) ) {
			/**
			 ...
			 */
			register_block_type( COE_BOARDS_PLUGIN_BLOCKS . $block . '/block.json' );
		}
	}
}
 ...
 * @since 0.1.1
 */
function coe_boards_get_blocks() {
	// Check for options.
	$blocks  = get_option( 'coe_boards_blocks' );
	$version = get_option( 'coe_boards_blocks_version' );

	if ( empty( $blocks ) || version_compare( COE_BOARDS_VERSION, $version ) || ( function_exists( 'wp_get_environment_type' ) && 'production' !== wp_get_environment_type() ) ) {
		$blocks = scandir( COE_BOARDS_PLUGIN_BLOCKS );
		$blocks = array_values( array_diff( $blocks, array( '..', '.', '.DS_Store' ) ) );

		// Update our options.
		update_option( 'coe_boards_blocks', $blocks );
		update_option( 'coe_boards_blocks_version', COE_BOARDS_VERSION );
	}
 ...
 * @since 0.1.1
 */
function coe_boards_block_category( $block_categories ) {

	$block_categories = array_merge(
		array(
			array(
				'slug'  => 'coe-boards-blocks',
				'title' => __( 'COE Blocks', 'coe-boards-blocks' ),
			),
		),
		$block_categories,
	);
 ...
 * @since 0.1.1
 */
function coe_boards_block_no_fields_msg( $message, $block_name ) {
